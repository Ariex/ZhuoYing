...
- **录制 GIF**（`F4` / 工具条 ⏺）：框选区域后活屏录制（24fps、含光标、
  相同画面自动合并时长），红框与控制条不入镜；完成存 GIF 到设定目录；
  流式编码内存占用与时长无关
...
`--test-record "x,y,w,h[,fps[,毫秒]]"`（录屏核心自测：区域内自带变色动画源，
录完输出帧数/大小到 `%TEMP%\zhuoying-record-test.txt`）、
`--test-record-ui "x,y,w,h[,毫秒]"`（完整录制 UI 流程，自动完成落盘）。
